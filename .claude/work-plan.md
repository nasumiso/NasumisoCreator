...
#### 実装フェーズ1: 基本構造とタブ1（画像準備）
- [ ] app.pyの基本構造作成（Gradio 3タブレイアウト）
- [ ] タブ1: 画像準備タブのUI構築
  - [ ] フォルダ選択UI（デフォルト: projects/nasumiso_v1/1_raw_images）
  - [ ] 「変換開始」ボタン
  - [ ] リアルタイム進捗表示エリア
- [ ] タブ1: 既存スクリプト統合ロジック
  - [ ] prepare_images.pyの実行処理
  - [ ] auto_caption.pyの実行処理（しきい値0.35固定）
  - [ ] add_common_tag.pyの実行処理（nasumiso_style固定）
  - [ ] 進捗表示の実装（ファイル名、進捗%）
  - [ ] エラーハンドリング（ファイル不存在、処理失敗）
