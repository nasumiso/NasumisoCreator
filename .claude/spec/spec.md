...
| 2025-11-08 | Gradio WebUI (画像準備タブ) | 画像前処理パイプラインのWebUI実装 |
| 2025-11-11 | Gradio WebUI (追加タグ機能) | フォルダごとに追加タグを指定できる機能を実装 |

---

## Gradio WebUI

### 8. なすみそLoRA学習アシスタントWebUI（app.py）

**目的**: 画像前処理からタグ編集、学習まで、ブラウザ上で操作できるWebインターフェースを提供する

**できること**:
- Webブラウザで操作できるGradioベースのUI
- 画像準備タブで一括前処理を実行
- タグ編集タブで個別のタグ編集（未実装）
- 学習・モデル管理タブ（未実装）
...
2. **ブラウザでアクセス**:
   ...
   - ポート7860が使用中の場合、自動的に7861、7862と番号を変えて起動

3. **画像準備タブの使い方**:
   - フォルダパス入力欄にデフォルトで `projects/nasumiso_v1/1_raw_images` が設定されている
   - 「追加タグ」列に学習用の追加タグを入力（カンマ区切り、例: `chara_nasumiso`）
   - 画像枚数ボタンをクリックすると、フォルダ内の画像一覧が表示される
   - 「🚀 変換開始」ボタンをクリックすると以下が自動実行される:
     1. 画像のリサイズと連番リネーム（512x512）
     2. WD14 Taggerで自動タグ付け（しきい値: 0.35）
     3. 共通タグ追加（`nasumiso_style` + 追加タグ）
   - 進捗状況がリアルタイムで表示される

**いつ使う？**:
- なすみそから画像を受け取った後、一括で前処理を実行したい時
- コマンドライン操作をせずにブラウザで作業したい時
- 将来的になすみそさん自身が操作する際の準備として

**ポイント**:
- **追加タグ機能**: `nasumiso_style`は固定で追加されるが、追加タグ欄にカンマ区切りで追加タグを指定可能
  - 例: `chara_nasumiso` と入力すると、`nasumiso_style, chara_nasumiso` が全画像に追加される
  - 空欄の場合は `nasumiso_style` のみ追加される
- **複数フォルダ対応**: 将来的に複数フォルダを追加できるよう、テーブル形式で設計（現在は1行のみ使用）
- **クロスプラットフォーム**: Mac/Windows両環境で動作
- **エラー表示**: ファイルが見つからない、変換に失敗した場合はわかりやすいメッセージが表示される
